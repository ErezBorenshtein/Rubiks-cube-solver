...
class ColorDetection:
    ...
    def __init__(self):
        ...
        #this color palette contains the *actual* bgr's of every color on the cube
    ...
    def get_bgr_color(self,color_name_input):
        for color_name, color_bgr in self.cube_color_palette.items():
            if color_name_input == color_name:
                return color_bgr
        return 0

    def get_next_color(self,bgr):
        for color_name, color_bgr in self.cube_color_palette.items():
            if tuple([int(c) for c in bgr]) == color_bgr:
                index = 0
                for prominent_color,_ in self.prominent_color_palette.items():
                    if color_name == prominent_color:
                        return list(self.prominent_color_palette)[(index+1)%6]
                    index +=1
