...
class Webcam:
    ...
    def find_contours2(self,contours):
    ...
    def find_contours(self, dilatedFrame,contours_in):
    ...
    def predefined_contours(self,is_reversed = False):
        delta = 110
        x1 = 190
        x2 = x1+delta
        x3 = x2+delta
        ...
        y3 = y2+delta
        if (is_reversed):
            return [(x3, y3, 50, 50), (x2, y3, 50, 50), (x1, y3, 50, 50), (x3, y2, 50, 50),(x2, y2, 50, 50),(x1, y2, 50, 50), (x3, y1, 50, 50),(x2, y1, 50, 50),(x1, y1, 50, 50)]
        else:
            return [(x1, y1, 50, 50), (x2, y1, 50, 50), (x3, y1, 50, 50), (x1, y2, 50, 50), (x2, y2, 50, 50), (x3, y2, 50, 50), (x1, y3, 50, 50), (x2, y3, 50, 50), (x3, y3, 50, 50)]
    ...
    def rearrange_side(self,preview_state,direction):
        new_snapshot = [preview_state[0],preview_state[1],preview_state[2],preview_state[3],preview_state[4],preview_state[5],preview_state[6],preview_state[7],preview_state[8]]
        if (direction == DIRECTION_FLIP_HORIZONTAL):
            new_snapshot = [preview_state[2],preview_state[1],preview_state[0],preview_state[5],preview_state[4],preview_state[3],preview_state[8],preview_state[7],preview_state[6]]
        if (direction == DIRECTION_FLIP_VERTICAL):
            new_snapshot = [preview_state[6],preview_state[7],preview_state[8],preview_state[3],preview_state[4],preview_state[5],preview_state[0],preview_state[1],preview_state[2]]
        return new_snapshot
    ...
    def draw_2d_cube_state(self, mouse_point=0):
        """
        ...
        mouse_point - in case function is provided with a point and needs to return the color and square it refers to
        """
        clicked_side = 0
        clicked_index = 0
        ...
        for side, (grid_x, grid_y) in grid.items():
            ...
            for row in range(3):
                for col in range(3):
                    ...
                    #if mouse was clicked within this square - save it and return it later
                    if mouse_point != 0 and mouse_point[0]>= x1 and mouse_point[0]<=x2 and mouse_point[1] >= y1 and mouse_point[1]<=y2:
                        clicked_side = side
                        clicked_index = index
        ...
        return  (clicked_side,clicked_index)
    
    def get_side_color(self,side):
        face_colors_nums = self.result_state[side]
        face_colors_names = []
        for color_num in face_colors_nums:
            #face_colors_names.append(color_detector.convert_bgr_to_notation(color_num))
            face_colors_names.append(color_detector.get_closest_color(color_num)['color_name'])
        return face_colors_names   

    def get_result_notation(self):
        """Convert all the sides and their BGR colors to cube notation."""
        notation = dict(self.result_state).copy()

        notation = dict()
        for side, preview in self.result_state.items():
            side_items = list()
            for bgr in preview:
                side_items.append((bgr[0],bgr[1],bgr[2]))
            notation[side] = side_items
    ...
    #turn camera on and store frame
    ...
    def waitForKey(self,message):
        ...
        while True:
            ...
            cv2.imshow("Cube Solver", self.frame)
            if (key == ord('a')):
                break

    def next_bgr(bgr_color):
        pass

    def update_square(self,mouse_click):
        side, index = self.draw_2d_cube_state(mouse_click)
        if list(self.result_state).index(side) != ValueError:
            bgr_color = self.result_state[side][index]
            next_color = color_detector.get_next_color(bgr_color)
            next_color_bgr = color_detector.get_bgr_color (next_color)
            self.result_state[side][index] = next_color_bgr

    #reads one face and update result_state
    def read_one_face(self,is_reverse = False):
        print ("Start reading face")
        start_time = time.time()
        while time.time() - start_time < time_to_read_face:
            _, frame = self.cam.read()
            self.frame = frame
            key = cv2.waitKey(10) & 0xff

            contours = self.predefined_contours(is_reverse)
        ...
        #take a snapshot of the face and store it
    ...
    def refresh_image(self):
        ...
        contours = self.predefined_contours(False)
    ...
    def run(self):
        ...
        while True:
            ...
            contours = self.predefined_contours(is_reversed)
            #contours = self.find_contours2(contours)
            #if (len(contours) != 0):
            #    print (len(contours))
        ...
        #implemented in FinalizeCamera instead
        #self.cam.release()
        #cv2.destroyAllWindows()
        ...
        if not self.scanned_successfully():
            return E_INCORRECTLY_SCANNED
        color_names = self.get_side_color('yellow')
